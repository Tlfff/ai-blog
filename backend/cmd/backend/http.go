...
type httpApplication struct {
	...
	commentLike        *appjob.CommentLikeRebuildJob          // commentLike 是评论点赞缓存与计数重建任务。
	interactionRepair  *appjob.ArticleInteractionReconcileJob // interactionRepair 是文章互动投影对账任务。
...
func (app *httpApplication) Run(ctx context.Context) error {
	...
	if err := leo.MutilRunner(app.server, app.reconciler, app.userSessionCleanup, app.hotRank, app.likeCache, app.commentLike, app.interactionRepair, app.viewEvents).Run(ctx); err != nil {
		return fmt.Errorf("运行博客 HTTP 应用: %w", err)
	}
	return nil
}
...
//   - commentLike：评论点赞缓存与计数重建任务。
//   - interactionRepair：文章评论数和点赞数投影对账任务。
//   - viewEvents：文章浏览事件异步发送任务。
func newApp(cfg *conf.Config, httpServer ginhttp.RegisterServer, reconciler *appjob.ArticleDeletionReconciler, userSessionCleanup *appjob.UserSessionCleanupJob, hotRank *appjob.ArticleHotRankJob, likeCache *appjob.ArticleLikeCacheRebuildJob, commentLike *appjob.CommentLikeRebuildJob, interactionRepair *appjob.ArticleInteractionReconcileJob, viewEvents *eventstream.ArticleViewPublisher) *httpApplication {
	// 1. 初始化 HTTP 中间件和生成路由
	...
	return &httpApplication{server: httpServers, reconciler: reconciler, userSessionCleanup: userSessionCleanup, hotRank: hotRank, likeCache: likeCache, commentLike: commentLike, interactionRepair: interactionRepair, viewEvents: viewEvents}
}
