...
// ArticleLikeEventOutbox 表示文章点赞集成事件的事务 Outbox 记录。
type ArticleLikeEventOutbox struct {
	EventID       string     `xorm:"'event_id' pk varchar(64)"`      // EventID 是事件幂等标识。
	AggregateID   uint64     `xorm:"'aggregate_id'"`                 // AggregateID 是点赞关系标识。
	EventType     string     `xorm:"'event_type' varchar(64)"`       // EventType 是稳定事件类型。
	Version       int64      `xorm:"'version'"`                      // Version 是点赞关系版本。
...
// TableName 返回文章点赞事件 Outbox 表名。
func (ArticleLikeEventOutbox) TableName() string { return "article_like_event_outbox" }

// CommentLikeEventOutbox 表示评论点赞集成事件的事务 Outbox 记录。
type CommentLikeEventOutbox struct {
	EventID       string     `xorm:"'event_id' pk varchar(64)"`      // EventID 是事件幂等标识。
	AggregateID   uint64     `xorm:"'aggregate_id'"`                 // AggregateID 是点赞关系标识。
	EventType     string     `xorm:"'event_type' varchar(64)"`       // EventType 是稳定事件类型。
	Version       int64      `xorm:"'version'"`                      // Version 是点赞关系版本。
...
// TableName 返回评论点赞事件 Outbox 表名。
func (CommentLikeEventOutbox) TableName() string { return "comment_like_event_outbox" }
