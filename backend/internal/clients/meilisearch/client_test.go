...
func TestClientAppendsPublishedFilterAndFormatsResults(t *testing.T) {
	// 1. 捕获请求并验证客户端不能覆盖已发布状态过滤
	client := NewClient("http://meili.test/", "secret")
	client.httpClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		if r.URL.String() != "http://meili.test/indexes/articles/search" || r.Header.Get("Authorization") != "Bearer secret" {
			t.Fatalf("url=%s authorization=%q", r.URL, r.Header.Get("Authorization"))
		}
		var payload request
		if err := json.NewDecoder(r.Body).Decode(&payload); err != nil {
			t.Fatal(err)
		}
		if payload.Filter != "status = 3" || payload.Offset != 10 || payload.Limit != 10 {
...
func TestClientNeverReturnsUnpublishedHits(t *testing.T) {
	...
	client := NewClient("http://meili.test", "")
...
// TestClientReturnsUnavailableForTransportAndResponseFailures 验证配置、传输、状态码和解码失败均保留错误分类。
func TestClientReturnsUnavailableForTransportAndResponseFailures(t *testing.T) {
	...
	cases := []struct {
		...
		baseURL    string // baseURL 是 Meilisearch 测试地址。
		statusCode int    // statusCode 是 Meilisearch 返回的 HTTP 状态码。
		body       string // body 是 Meilisearch 返回的响应体。
		err        error  // err 是传输层预设错误。
	}{
		{name: "missing endpoint"},
		{name: "transport", baseURL: "http://meili.test", err: errors.New("connection reset")},
		{name: "http status", baseURL: "http://meili.test", statusCode: http.StatusBadGateway, body: "bad gateway"},
		{name: "invalid json", baseURL: "http://meili.test", statusCode: http.StatusOK, body: "{"},
	}
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			client := NewClient(testCase.baseURL, "")
			client.httpClient = &http.Client{Transport: roundTripFunc(func(*http.Request) (*http.Response, error) {
				if testCase.err != nil {
